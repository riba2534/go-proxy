...
func handShake(conn net.Conn) (err error) {
	...
	// make sure we get the nmethod field 读取的数据至少位1位，ReadAtLeast是至少读多少数据,n是一共读了多少字节
...
func pipeWhenClose(conn net.Conn, target string) {
	...
	rep := make([]byte, 256)
	rep[0] = 0x05 //协议版本
	rep[1] = 0x00 // success  状态码
	rep[2] = 0x00 //RSV 保留位0x00

	//IP
	if tcpAddr.Zone == "ip6" { //目标地址类型位
		rep[3] = 0x04 //IPv6
	} else {
		rep[3] = 0x01 //IPv4
	}
	...
	pindex := 4
	for _, b := range ip { //ip类型十个slice，把数据放进字节中
		rep[pindex] = b
		pindex += 1
	}
	rep[pindex] = byte((tcpAddr.Port >> 8) & 0xff) //把端口号转换成byte来表示
	...
	go netCopy(conn, remoteConn) // conn->remote

	// Copy remote to local
	netCopy(remoteConn, conn) // remote->conn
}
...
func main() {
	...
	verbose := flag.Bool("v", false, "Every request log to stdout.")
